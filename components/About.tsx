...
interface Feature { t: string; d: string }
...
interface Props {
  ...
  titulo1?: string; titulo2?: string
  anos?: string; anosLabel?: string
  badge?: string
  features?: Feature[]
}

export default function About({ texto1, texto2, titulo1, titulo2, anos, anosLabel, badge, features }: Props) {
  const feats = (features && features.length > 0) ? features : DEFAULT_FEATURES
  return (
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 80, alignItems: 'center' }}>
        ...
        <Reveal direction="left">
          <div style={{ position: 'relative' }}>
            ...
            />
            <div style={{ position: 'absolute', bottom: -24, right: -24, background: 'var(--accent)', padding: '28px 32px', borderRadius: 2 }}>
              <div style={{ fontFamily: 'var(--font-heading)', fontSize: 44, fontWeight: 300, color: '#fff', lineHeight: 1 }}>{anos || '12'}</div>
              <div style={{ fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.8)', marginTop: 6 }}>{anosLabel || 'anos de mercado'}</div>
            </div>
          </div>
        </Reveal>
        ...
        <Reveal direction="right">
          <div>
            ...
            <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: 'clamp(32px, 4vw, 48px)', fontWeight: 300, color: 'var(--ink)', lineHeight: 1.15, marginBottom: 28 }}>
              {titulo1 || 'Mais de uma década'}<br />
              <em style={{ fontStyle: 'italic' }}>{titulo2 || 'transformando lares'}</em>
            </h2>

            <p style={{ fontSize: 15, color: 'var(--muted)', lineHeight: 1.8, marginBottom: 20 }}>{texto1 || D1}</p>
            <p style={{ fontSize: 15, color: 'var(--muted)', lineHeight: 1.8, marginBottom: 44 }}>{texto2 || D2}</p>
            ...
            <a href="#contato" style={{ display: 'inline-block', background: 'var(--ink)', color: '#fff', padding: '14px 36px', borderRadius: 2, fontSize: 12, letterSpacing: '0.1em', textDecoration: 'none', textTransform: 'uppercase', fontWeight: 500, transition: 'background 0.2s' }}
